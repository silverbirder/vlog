  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center gap-3">
        <span className="text-sm text-muted-foreground">通知権限: {notificationPermission}</span>
      ...
      <div className="flex items-center gap-3">
        ...
        <Button disabled={microphoneStatus === "granted"} onClick={() => void requestMicrophone()}>
          権限をリクエスト
        </Button>
        <Button disabled={microphoneStatus !== "granted"} onClick={() => void validateMicrophone()}>
          マイクテスト
        </Button>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <Button disabled={cameraStatus === "granted"} onClick={() => void requestCamera()}>
          権限をリクエスト
        </Button>
        <Button disabled={cameraStatus !== "granted"} onClick={() => void validateCamera()}>
          カメラテスト
        </Button>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <Button disabled={screenStatus === "granted"} onClick={() => void requestScreen()}>
          権限をリクエスト
        </Button>
        <Button disabled={screenStatus !== "granted"} onClick={() => void validateScreen()}>
          スクリーンテスト
        </Button>
      </div>
    </div>
